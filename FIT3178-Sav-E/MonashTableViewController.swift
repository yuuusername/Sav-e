//
//  MonashTableViewController.swift
//  FIT3178-SwiftSoup
//
//  Created by Michael Wybrow on 16/4/2022.
//
...
class MonashTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        Task {
            do {
                guard let monashURL = URL(string: "https://new.igashop.com.au/sm/pickup/rsid/52511/results?q=apple") else {
                    throw ParserError.invalidURL
                }
                let (data, _) = try await URLSession.shared.data(from: monashURL)
                ...
                for element: Element in cardElements.array() {
                    ...
                    if let formatName = formatName, let formatPrice = formatPrice, let svgURLString = svgURLString {
                        products.append(Product(name: formatName, price: formatPrice, jpgURLString: svgURLString))
                    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "BenefitCell", for: indexPath)
